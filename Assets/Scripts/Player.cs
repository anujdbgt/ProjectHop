...
public class Player : MonoBehaviour
{
        ...
        if (CheckTouchInput())
        {
            if (Touchscreen.current.position.x.ReadValue() > Screen.width / 2)
            {
                ...
                if (context.started && !touchingBottom && (CheckFrontCollision(rightMovement) || CheckFrontCollision(leftMovement)))
                {
                    ...
                    StartCoroutine(WallJumpAndSlide(rightMovement));
                }
                if (context.started)
                {
                    FlipToRight();
                ...
                if (context.started && slidingDone && touchingBottom && CheckFrontCollision(rightMovement))
                {
                ...
                if (context.started && slidingDone && touchingBottom && !CheckFrontCollision(rightMovement))
                {
                    StartCoroutine(JumpSildeProcess(xMaxDistance, yMaxDistance, rightMovement));
                }
        ...
        if (CheckTouchInput())
        {
            if(Touchscreen.current.position.x.ReadValue() < Screen.width / 2)
            {
                ...
                if (context.started && !touchingBottom && (CheckFrontCollision(rightMovement) || CheckFrontCollision(leftMovement)))
                {
                    Debug.Log("left jump");
                    //Resetting Time to Unsitck
                    StartCoroutine(WallJumpAndSlide(leftMovement));
                }
                if (context.started)
                {
                    FlipToLeft();
                ...
                if (context.started && slidingDone && touchingBottom && CheckFrontCollision(leftMovement))
                {
                ...
                if (context.started && slidingDone && touchingBottom && !CheckFrontCollision(leftMovement))
                {
                    StartCoroutine(JumpSildeProcess(xMaxDistance, yMaxDistance, leftMovement));
                }
                //Hold to jump higher and longer
                if (context.canceled && !touchingBottom)
                {
    ...
    IEnumerator WallJumpAndSlide(int movementDirection)
    {
        WallJump(movementDirection);
        yield return new WaitForSeconds(0.4f);
        yield return new WaitUntil(() => touchingBottom);
        Sliding(movementDirection);
    }
    IEnumerator JumpSildeProcess(float xDistance, float yDistance, int movementDirection)
    {
...
#region CollisionCheck
    ...
    void OnCollisionEnter2D(Collision2D col)
    {
        ...
        if (dir.y < 0 && col.gameObject.tag == "Ground")
        {
            touchingBottom = true;
        }
    }
